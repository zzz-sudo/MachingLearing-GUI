...
export function ChartWorkspace({ dataset, preview, charts, onCreateChart, onGenerateChart, onGetChartResult, getChartArtifactUrl }: ChartWorkspaceProps) {
  ...
  const savedCharts = charts.filter((chart) => chart.status === "succeeded" && chart.artifactIds.length > 0);
  useEffect(() => {
  ...
  async function saveAndGenerate() {
    const chart = await onCreateChart({ name: name.trim(), chartType, datasetId: dataset!.id, xColumn, yColumns, options: { theme: "light", showLegend: true } });
    setGenerating(true);
    try {
    ...
    <div className="workspace-scroll chart-workspace">
      ...
      <div className="chart-workspace-grid">
        <section className="chart-canvas-section">
          ...
          {charts.length > 0 ? <div className="saved-chart-panel"><div className="saved-chart-heading"><span>已生成图形</span><strong>{savedCharts.length}/{charts.length}</strong></div><div className="saved-chart-list">{charts.map((chart) => <button className={chart.id === selectedSavedChartId ? "saved-chart-item active" : "saved-chart-item"} key={chart.id} type="button" onClick={() => setSelectedSavedChartId(chart.id)}><span>{chart.name}</span><small>{chart.status}</small></button>)}</div>{selectedSavedChart && savedJobId && savedRelativePath ? <div className="saved-chart-preview"><iframe className="chart-iframe" title={`${selectedSavedChart.name} HTML 预览`} src={getChartArtifactUrl(savedJobId, savedRelativePath)} sandbox="allow-scripts" /><a className="artifact-download-button" href={getChartArtifactUrl(savedJobId, savedRelativePath)} target="_blank" rel="noreferrer"><ExternalLink aria-hidden="true" size={14} />打开当前图形</a></div> : <p className="chart-empty-hint">训练完成后，诊断图会在这里显示。</p>}</div> : null}
        ...
        <section className="chart-config-section">
          ...
          <div className="chart-config-form">
            ...
            <label><span>Y 轴字段</span><select multiple size={Math.min(5, Math.max(2, numericColumns.length))} value={yColumns} onChange={(event) => setYColumns(Array.from(event.target.selectedOptions, (option) => option.value))}>{numericColumns.map((column) => <option key={column.name} value={column.name}>{column.name}</option>)}</select></label>
            ...
            <button className="primary-button" disabled={generating || !dataset || !xColumn || yColumns.length === 0 || !name.trim()} type="button" onClick={() => void saveAndGenerate()}><BarChart3 aria-hidden="true" size={15} />{generating ? "正在生成" : "保存并生成图形"}</button>
