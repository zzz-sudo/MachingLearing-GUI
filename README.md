...
算法目录现在额外记录 `status`、依赖包和推荐图形模板。当前目录中的分类、回归、聚类、方差分析、假设检验、降维和数据探索算法均已接入 Worker，并由对应的执行测试覆盖。`available` 表示 Runner、输入校验、结果产物和测试已经闭环；前端仍保留对未来 `planned` 或 `disabled` 条目的禁用逻辑。

当前算法目录共 58 个可运行条目，包含 KNN、朴素贝叶斯、支持向量机、判别分析、梯度提升、ElasticNet、Lasso、Huber、SVR、Bagging、LightGBM、Poisson、分位数回归、K-Medoids、Fuzzy C-Means、OPTICS、谱聚类、t 检验、卡方检验、PCA、t-SNE、Truncated SVD、UMAP、缺失值分析和相关性分析等。
...
序列深度学习 Runner 已经实现 RNN、LSTM 和 GRU 的序列回归与序列分类。序列任务必须指定时间字段、目标字段、特征字段和窗口长度，可以指定实体分组字段。每个实体先按时间升序排列，再按目标时刻进行训练区间和测试区间划分。标准化器只在训练区间拟合，窗口不会把测试目标或未来记录泄漏到训练数据。回归输出 MAE、MSE、R squared 和预测预览，分类输出 accuracy、weighted F1 和预测预览。

PyTorch 产物 `model.pt` 包含网络单元类型、输入维度、隐藏层维度、层数、类别编码、标准化器均值和尺度以及 state dict；同目录的 `network.json` 保存窗口、预测步长、训练历史、时间切分说明、字段顺序和设备信息。模型重新加载时必须同时恢复标准化参数和网络配置，不能只加载权重后使用未经同样处理的原始数据。序列测试覆盖三种循环单元、回归和分类、时间倒序输入恢复、固定训练轮数和产物元数据。
